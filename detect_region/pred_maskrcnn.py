    ...
    for ele in range(len(outputs)):
        for lab in labels:
            ...
            if sel[0].any():
                ...
                mask = outputs[ele]['masks'][sel]
                mask = mask[0].squeeze()#TODO get best
                #threshold: try diff values
                mask[wholepredlbl[ele]!=0]=0
                mask[mask>0.5] = lab
                mask[mask<0.5] = 0 
                wholepredlbl[ele]+=mask
...
def naive_test(testloader, model1, loss_func=None, postpr='no', save_incorr=False):
    ...
    with torch.no_grad():
        for data in testloader:
            ...
            x, pred_sgm=get_mask(images, model1)
...
def test_pred(model1, test_loader, experiment_name="",
loss_func=None, sgm_train=True):
    ...
    naive_test(test_loader, model1, loss_func)
